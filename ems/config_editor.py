...
FIELD_DESCRIPTIONS = {
    ...
    "controllable_loads[].power_topic": "MQTT-Topic der real gemessenen Leistung. Damit werden Ist-Lauf und Soll/Ist-Leistung im Dashboard geprüft.",
    "controllable_loads[].feedback_on_threshold_w": "Ab dieser gemessenen Leistung gilt die steuerbare Last als tatsächlich eingeschaltet.",
...
<style>
:root{color-scheme:light;--bg:#eef2f6;--card:#fff;--text:#20252b;--muted:#434a54;--line:#e3e8ee;--r-card:14px;--r-ctl:9px;--s0:2px;--s1:4px;--s2:8px;--s3:12px;--s4:16px;--s5:24px;--t0:11px;--t1:12px;--t2:14px;--t3:17px;--t4:22px;--shadow:0 1px 2px rgba(20,35,55,.05),0 4px 16px rgba(20,35,55,.06);--input:#fff;--blue:#1769c2;--soft:#f5f7f9;--surface:#f1f4f8;--seitenbreite:1800px;--muted-2:#2f353c;--focus:#1769c2;--bad:#b52d28;--ok:#217a41;--warn:#7f631b}
html.dark{color-scheme:dark;--bg:#10171e;--card:#18212b;--text:#e7edf4;--muted:#bcc7d3;--line:#33414f;--shadow:0 1px 2px rgba(0,0,0,.30),0 4px 16px rgba(0,0,0,.28);--input:#202e3a;--blue:#338be0;--soft:#202e3a;--surface:#202b36;--muted-2:#ccd6e0;--focus:#4ea1f0;--bad:#ff8c87;--ok:#75ce91;--warn:#e5cb74}
...
.top-in{max-width:var(--seitenbreite);margin:auto;padding:var(--s3) var(--s4);display:flex;align-items:center;gap:var(--s3)}.back{color:var(--blue);text-decoration:none;font-weight:700}.title{flex:1}.title h1{font-size:var(--t3);margin:0}.title small{color:var(--muted)}
button,.button{border:1px solid var(--line);border-radius:var(--r-ctl);background:var(--soft);color:var(--text);padding:var(--s2) var(--s3);font:inherit;cursor:pointer}button.primary{background:var(--blue);border-color:var(--blue);color:white;font-weight:700}button.danger{color:var(--bad)}button:disabled{opacity:.55;cursor:wait}
main{max-width:var(--seitenbreite);margin:auto;padding:var(--s4) var(--s4) 100px}.notice{padding:var(--s3) var(--s3);border:1px solid var(--warn);background:#fff7da;color:var(--warn);border-radius:var(--r-ctl);margin-bottom:var(--s3)}html.dark .notice{background:#3a3219;color:#e5cb74;border-color:#6a5925}
.tools{display:grid;grid-template-columns:minmax(220px,1fr) auto;gap:var(--s3);margin-bottom:var(--s3)}.tools input{width:100%}
details.panel{background:var(--card);border:1px solid var(--line);border-radius:var(--r-card);margin:var(--s2) 0;overflow:hidden;box-shadow:var(--shadow)}details.panel>summary{list-style:none;padding:var(--s4) var(--s4);cursor:pointer;display:flex;align-items:center;gap:var(--s3)}details.panel>summary::-webkit-details-marker{display:none}.panel-title{font-weight:750;font-size:var(--t2)}.panel-desc{display:block;color:var(--muted);font-size:var(--t1);margin-top:var(--s0)}.panel-count{margin-left:auto;color:var(--muted);font-size:var(--t1)}.panel-body{border-top:1px solid var(--line);padding:var(--s4)}
.field-row{display:grid;grid-template-columns:minmax(190px,.8fr) minmax(260px,1.4fr);gap:var(--s4);padding:var(--s2) var(--s1);border-bottom:1px solid color-mix(in srgb,var(--line) 65%,transparent)}.field-row:last-child{border-bottom:0}.field-label b{display:block;overflow-wrap:anywhere}.field-label small{display:block;color:var(--muted);margin-top:var(--s0);line-height:1.35}.path{font:10px ui-monospace,monospace;color:var(--muted)}
input,select,textarea{width:100%;border:1px solid var(--line);border-radius:var(--r-ctl);background:var(--input);color:var(--text);padding:var(--s2) var(--s2);font:inherit}input[type=checkbox]{width:22px;height:22px;accent-color:var(--blue)}.bool-wrap{min-height:38px;display:flex;align-items:center}.secret-wrap{display:flex;gap:var(--s1)}.secret-wrap button{padding:var(--s1) var(--s2)}
fieldset.group{border:1px solid var(--line);border-radius:var(--r-ctl);margin:var(--s3) 0;padding:var(--s2) var(--s3) var(--s3)}fieldset.group>legend{padding:0 var(--s2);font-weight:700}.group-actions,.item-actions,.load-add{display:flex;gap:var(--s2);justify-content:flex-end;margin-top:var(--s2)}.list-item,.load-card{border:1px solid var(--line);background:var(--soft);border-radius:var(--r-ctl);padding:var(--s3);margin:var(--s2) 0}.load-head{display:flex;align-items:center;gap:var(--s2)}.load-head b{flex:1;font-size:var(--t2)}.badge{font-size:var(--t1);color:var(--blue);border:1px solid color-mix(in srgb,var(--blue) 55%,var(--line));border-radius:20px;padding:var(--s0) var(--s2)}
.array-scalar{display:grid;grid-template-columns:1fr auto;gap:var(--s2);margin:var(--s1) 0}.empty{color:var(--muted);padding:var(--s2)}.status{position:fixed;z-index:25;left:50%;bottom:var(--s4);transform:translateX(-50%);width:min(920px,calc(100% - 24px));display:flex;align-items:center;gap:var(--s3);padding:var(--s3) var(--s3);background:var(--card);border:1px solid var(--line);border-radius:var(--r-card);box-shadow:0 8px 30px #0003}.status-text{flex:1}.status-text small{display:block;color:var(--muted);margin-top:var(--s0)}.status.ok{border-color:#64b87d}.status.err{border-color:#d56b67}.hidden{display:none!important}
@media(max-width:700px){.top-in{padding:var(--s3)}.title small{display:none}.title h1{font-size:var(--t3)}main{padding:var(--s3) var(--s2) 105px}.tools{grid-template-columns:1fr}.field-row{grid-template-columns:1fr;gap:var(--s2)}.panel-body{padding:var(--s3)}.field-row{padding:var(--s3) var(--s0)}.top button{padding:var(--s2)}.desktop-label{display:none}.status{bottom:8px;flex-wrap:wrap}.status-text{flex-basis:100%}.status button{flex:1;min-height:42px}}
</style></head><body>
