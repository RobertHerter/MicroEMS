...
class HomeyMqttPublisher:
    ...
    def _lane_command(self, lane: dict, planned_on: bool) -> int:
        ...
        Heiz-FREIGABE: liegt die Ist-Temperatur auf/über max_c, hält der
        ...
        UND T unter max_c, d.h. der Thermostat WÜRDE sonst heizen)."""
        if planned_on:
            return 1
        if not (lane.get("thermostat") and lane.get("temp_signal")):
            return 0
        t_ist = self.get_load_temp(lane["temp_signal"])
        # Massgeblich ist max_c - bei einem Geraet mit eigenem Thermostat SEIN
        # Abschaltpunkt. Darunter wuerde es weiterheizen, dort darf die Freigabe
        # also NICHT gehalten werden (genau das lief eine Nacht durch und zog
        # den Akku von 97 % auf 5 %, obwohl der Plan 0 W vorsah).
        if t_ist is None or float(t_ist) < lane.get("max_c", 1e9):
            return 0
        t_ist = float(t_ist)
        # Die Freigabe darf nur gehalten werden, solange der geraeteeigene
        # Thermostat das Heizen wirklich verhindert. Meldet die Stufe real
        # LEISTUNG, ist die Annahme fuer dieses Geraet falsch -> abschalten.
        try:
