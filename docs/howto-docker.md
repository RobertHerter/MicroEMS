...
curl -fsSL https://raw.githubusercontent.com/RobertHerter/MicroEMS/v3.5.1/config.example.yaml \
     -o config/config.yaml
curl -fsSL https://raw.githubusercontent.com/RobertHerter/MicroEMS/v3.5.1/docker-compose.yml \
...
services:
  ems:
    image: ghcr.io/robertherter/microems:3.5.1
    # build: .        <- entfernen
  scheduler:
    image: ghcr.io/robertherter/microems:3.5.1
    # build: .        <- entfernen
...
docker run --rm -v ./config:/app/config -v ./data:/app/data \
  ghcr.io/robertherter/microems:3.5.1 --config /app/config/config.yaml --check
```
...
DOCK="docker run --rm -v ./config:/app/config -v ./data:/app/data \
      --entrypoint python ghcr.io/robertherter/microems:3.5.1"
