...
curl -fsSL https://raw.githubusercontent.com/RobertHerter/MicroEMS/v3.2.0/config.example.yaml \
     -o config/config.yaml
curl -fsSL https://raw.githubusercontent.com/RobertHerter/MicroEMS/v3.2.0/docker-compose.yml \
...
services:
  ems:
    image: ghcr.io/robertherter/microems:3.2.0
    # build: .        <- entfernen
  scheduler:
    image: ghcr.io/robertherter/microems:3.2.0
    # build: .        <- entfernen
...
docker run --rm -v ./config:/app/config -v ./data:/app/data \
  ghcr.io/robertherter/microems:3.2.0 --config /app/config/config.yaml --check
```
...
DOCK="docker run --rm -v ./config:/app/config -v ./data:/app/data \
      --entrypoint python ghcr.io/robertherter/microems:3.2.0"
