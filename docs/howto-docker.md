...
curl -fsSL https://raw.githubusercontent.com/RobertHerter/MicroEMS/v3.0.2/config.example.yaml \
     -o config/config.yaml
curl -fsSL https://raw.githubusercontent.com/RobertHerter/MicroEMS/v3.0.2/docker-compose.yml \
...
services:
  ems:
    image: ghcr.io/robertherter/microems:3.0.2
    # build: .        <- entfernen
  scheduler:
    image: ghcr.io/robertherter/microems:3.0.2
    # build: .        <- entfernen
...
docker run --rm -v ./config:/app/config -v ./data:/app/data \
  ghcr.io/robertherter/microems:3.0.2 --config /app/config/config.yaml --check
```
...
DOCK="docker run --rm -v ./config:/app/config -v ./data:/app/data \
      --entrypoint python ghcr.io/robertherter/microems:3.0.2"
